  ...
  const introductions: Introduction[] = [
    {
      title: "About us",
      description:
        "現職（2021年8月〜）はReact.jsとTypescript、Next.js・GraphQLを使用したwebアプリケーションの開発を行っております。前職（2021年4月末）はwebサイト制作の受託開発でディレクター（7割）とフロントエンドエンジニア（3割）兼務しておりました。WordPressや静的なWebサイトがメインとなります、一部React.jsなどで仮想DOM操作のコンテンツも実装。キャッチアップは業務時間外でのものが多く実務ではフロントエンドエンジニアとしては未熟です。React.jsを使用するようなモダンなSPA開発はそれなりにできると思いますがが実務経験が乏しいのが現時点の要約になりますのでそれでもよければ。。という形となります。至らない点もありますが能動的なフットワークで技術のキャッチアップを行い仕事に活かしております。",
      action: "About us >",
      skillSet: false,
    },
    {
